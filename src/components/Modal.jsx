...
export const Modal = ({ numOfRolls, newGame }) => {
  const [lowestNumRolls, setLowestNumRolls] = useState(() => JSON.parse(localStorage.getItem("lowestNumRolls") || 0));
  const [highestNumRolls, setHighestNumRolls] = useState(() => JSON.parse(localStorage.getItem("highestNumRolls") || 1));

  useEffect(() => {
      console.log("Set Highest");

      if (numOfRolls !== 0 && (numOfRolls < lowestNumRolls || lowestNumRolls === 0)) {
